...
export default function Index() {
  ...
  const handleManualEntry = () => {
    // Ensure everything is blank, but keep quantity at 1 for convenience
    setAddBarcode('');
    setAddName('');
    setAddCompany('');
    setAddPrice('');
  ...
  const handleAddItemToCart = () => {
    if (!addName || !addPrice || !addQuantity) {
      Alert.alert("Missing Fields", "Please enter the required details (Name, Price, Quantity).");
      return;
    }

    const newItem = {
      code: addBarcode, // Will be empty string if added manually without barcode
      name: addName,
