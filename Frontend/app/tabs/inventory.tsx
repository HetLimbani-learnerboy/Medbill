...
// --- Configuration ---
const API_BASE_URL = 'http://YOUR_BACKEND_IP:5000/api/inventory'; 

// --- TypeScript Interfaces ---
interface InventoryItem {
  id: string; // Mapping medicine_id from backend
  name: string; // Mapping medicine_name from backend
  company: string;
  price: number;
  quantity: number;
}
...
export default function InventoryScreen() {
  ...
  const fetchInventory = async () => {
    try {
      ...
      Alert.alert("Error", "Failed to load inventory from server.");
      console.error(error);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSaveQuantity = async () => {
    if (selectedItem) {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/medicine-name/${selectedItem.name}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ quantity: isNaN(newQty) ? 0 : newQty })
        });
        ...
        if (response.ok) {
          ...
          Alert.alert("Update Failed", "Server rejected the update.");
        }
      } catch (error) {
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        Alert.alert("Error", "Failed to add medicine to database.");
      }
    } catch (error) {
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          ...
          >
            <View style={styles.modalContent}>
              ...
              <TextInput
                style={styles.input}
                placeholder="Company (e.g., Pharma)"
                value={newItem.company}
                onChangeText={(text) => setNewItem({...newItem, company: text})}
              />
              <View style={styles.row}>
